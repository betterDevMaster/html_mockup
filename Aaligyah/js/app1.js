...
// Load Analaytics Utilities
$.getScript('//cdn.shopify.com/s/files/1/0714/1079/t/73/assets/constants.js?v=11869785987179937712', function() {

});

function GAProductFromLineItem(lineItem, productOptions) {
 var product = {
        'name': lineItem.product_title,
        'id': lineItem.product_id,
        'price': (lineItem.price / 100).toString(),
        'brand': lineItem.vendor,
        'category': capitalize(lineItem.product_type),
   		'variant': lineItem.variant_title,
   		'quantity':lineItem.quantity,
  };

  var variantValues = lineItem.variant_options;
  var sizeOptionIndex = productOptions.indexOf(FANJOY_SIZE_OPTION_NAME);
  var colorOptionIndex = productOptions.indexOf(FANJOY_COLOR_OPTION_NAME);

  if (sizeOptionIndex >= 0) {
  	product[GA_CUSTOM_SIZE_DIMENSION] = capitalize(variantValues[sizeOptionIndex].toLowerCase());
  }
  if (colorOptionIndex >= 0) {
  	product[GA_CUSTOM_COLOR_DIMENSION] = capitalize(variantValues[colorOptionIndex].toLowerCase());
  }

  return product;
}

function trackCartUpdate(currentCartData, lineItemIndex, newQuantity, callback = undefined) {

  var lineItem = currentCartData.items[lineItemIndex-1];
  var priorQuantity = lineItem.quantity;

  // This should never be the case, investigate if it is
  if (newQuantity == priorQuantity) {
    debugger;
    return;
  }

  // Grab the full product so we can properly account for its custom dimensions
  $.ajax({
          url: '/products/' + lineItem.handle + '.js',
          dataType: 'json',
          cache: false,
          type: 'get',
    success: function (productData) {
      var GAProduct = GAProductFromLineItem(lineItem, productData.options.map(option => option.name));
      GAProduct.quantity = Math.abs(newQuantity - priorQuantity);

      var isProductAdd    = (newQuantity > priorQuantity);
      var ecommerceEvent  = isProductAdd ? GA_ADD_EVENT_NAME : GA_REMOVE_EVENT_NAME;
      var ecommerceAction = isProductAdd ? "add" : "remove";;

      var ecommerceObject = {};
      ecommerceObject[ecommerceAction] = { products: [GAProduct] }
      window.dataLayer.push({ event: ecommerceEvent, ecommerce: ecommerceObject });
...
var productFormListeners = {
  init: function(){
    ...
    // This is the (-) listener for the cart sidebar
    $(document).on("click", ".minus", function(e) {
      var quantity = parseInt($(this).next("input").val());
      quantity -= 1;
      $(this).next("input").val(quantity);

      // If we hit zero, we don't use the change trigger
      // because the entire input element gets deleted (I'm assuming).
      // Trigger the update through another path
      if (quantity == 0) {
        var lineID = parseInt($(this).next("input").attr("data-line-id"));
        ...
        $.ajax({
          ...
          cache: false,
          type: 'get',
          success: function (priorCartState) {
            trackCartUpdate(priorCartState, lineID, quantity, function () {
    ...
    // This listener is for the cart page quantity fields, we need to track updates before the form submits
    $(document).on("change", "#cart_form input[type='number']", function() {
      var newQuantity = parseInt($(this).val());
      var lineID = parseInt($(this).attr("data-line-id"));
      $.ajax({
        url: '/cart.js',
        dataType: 'json',
        cache: false,
        type: 'get',
        success: function (priorCartState) {
          trackCartUpdate(priorCartState, lineID, newQuantity, function () {
            $("#cart_form").submit();
          });
        }
      });
    ...
    // This listener is for the remove button on the cart page
    $(document).on("click", "#cart_form p[class='remove_item'] a", function(e) {
      var lineID = parseInt($(this).attr("data-line-id"));
      ...
      $.ajax({
        ...
        type: 'get',
        success: function (priorCartState) {
          trackCartUpdate(priorCartState, lineID, 0, function () {
            // Once we've logged everything, follow the link
...
/*============================================================================
  Collection
==============================================================================*/
var isTouchScreen = (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent));
function shouldToggleHoverState() {
  // No hover state for touchscreens
...
var collection = {
  ...
  showSecondaryImage: function(){
    $('body').on("mouseenter", ".thumbnail", function(e) {
      if (shouldToggleHoverState()) {
    ...
    $('body').on("mouseleave", ".thumbnail", function(e) {
      if (shouldToggleHoverState()) {
...
==============================================================================*/
  ...
  function ajaxUpdateCart(lineID, quantity, parent) {
    // Grab the prior state so we can track the changes before committing them
    $.ajax({
      url: '/cart.js',
      dataType: 'json',
      cache: false,
      type: 'get',
      success: function (priorCartState) {
        trackCartUpdate(priorCartState, lineID, quantity);
        $.ajax({
            url: '/cart/change.js',
            dataType: 'json',
            cache: false,
...
$(function() {
  ...
  if ($('.shopify-section.page-details-section').length){
    ...
    $('iframe[src*=youtube]:not(.shogun-video-embed), iframe[src*=vimeo]:not(.shogun-video-embed)').mediaWrapper({
      intrinsic: true
    });
  }
...
localSelectCallback = undefined;
var userSelectedVariant = undefined;
selectCallback = function(variant, selector) {
  ...
  if (variant && variant.featured_image && $product.is(":visible")) {
    // Don't tweak slider unless we're displaying a specific variant.
    var queryDict = {};
    try {
    ...
    if (userSelectedVariant !== undefined || queryDict["variant"] !== undefined) {
  ...
  if (localSelectCallback) {
      localSelectCallback(variant, selector);
  }
};
